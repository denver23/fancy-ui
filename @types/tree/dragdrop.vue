  ...
  .fancy-tree(:class="'fc-'+ (treelevel || 0) " v-if="data && data.length")
    ul(v-if="data.length")
      li(
        ...
        @dragover="onDragOver($event, v, ttree)"
        @dragstart="onDragStart($event, v, ttree)"
        @dragenter="onDragEnter($event, v, ttree)"
      )
        div.fc-item
          .fc-arrow(@click="onToggle(v)")
          .fc-name
            span
              input(
              ...
              span(v-else @click="onToggleName($event, index, v)") {{v[cfg.field]}}

          .fc-tool
            em.fc-pencil(v-if="cfg.editBtn && v.editBtn !== false" @click="onEdit($event, index, v)")
            em.fc-plus(v-if="cfg.insertBtn && v[cfg.field] && cfg.maxLevel > (treelevel || 0)" @click="onAdd($event, index, v)")
