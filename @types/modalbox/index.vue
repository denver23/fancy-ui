  ...
  .fancy-modalbox(:class="cfg.overlay ? 'fc-mask' : 'fc-nomask'" v-if="cfg.content")
    div(:style="cfg.style" ref="box")
      ...
      .fc-btns(v-if="cfg.confirm || cfg.cancel")
        span(:class="{'fc-sending': sending === 'confirm'}" v-if="cfg.confirm" @click="onClick(1)")
          em {{cfg.confirm}}
        span(:class="{'fc-sending': sending === 'cancel'}" v-if="cfg.cancel" @click="onClick(0)")
          em {{cfg.cancel}}
</template>
